...
const SipsikuPage = props => {
  const riigidNimedega = props.data.allImageSharp.edges.map(
    ({ node }, index) => {
  ...
  const riigid = Array.from(riigidGroup, ([riik, pildid]) => [
    riik,
    pildid.map(pilt => pilt[1]),
  ])
  const riigiPildid = riigid.map(([riik, pildid]) => (
    <Riik nimi={riik} pilt={pildid} />
  ))
  return (
    <div>
      <h2>Sipsiku leht</h2>
